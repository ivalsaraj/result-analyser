<html lang="en">
	<head>
		...
		<meta name="viewport" content="width=<?php echo $this->router->fetch_method()!='index'?"1350":'1350';?>">
	...
	echo "\n";
?>
		<title><?php $t=(!empty($meta['title']) ? $meta['title'].' | ' : '')."MCA Result Analyser"; echo $t;?>
		</title>
		<meta name="image" content="<?php echo base_url('assets/icon.png') ?>" />
  <link rel="canonical" href="<?php echo base_url().uri_string() ?>">
  <meta name="og_title" property="og:title" content="<?php echo $t ?>" />
  <meta name="og_site_name" property="og:site_name" content="MCA Result Analyser" />
  <meta name="og_url" property="og:url" content="<?php echo base_url().uri_string() ?>" />
  <meta name="og_description" property="og:description" content="<?php echo $meta['description'] ?>" />
  <meta name="og_image" property="og:image" content="<?php echo base_url('assets/icon.png') ?>" />
<?php
	if(!empty($canonical))
	{
		echo "\n\t\t";
...
<header class="main-header">               
	...
	<script>
          ...
          <div id="navbar-collapse" class="collapse navbar-collapse">
            <ul class="nav navbar-nav"><?php $ur=$this->router->fetch_class(); ?>
              ...
              <li><a href="<?php echo base_url().'result/'.$ur.'/subjectwiserank'; ?>">Subject wise Rank</a></li>
              <li><a href="<?php echo base_url().'result/'.$ur.'/changelog'; ?>">What's New?</a></li>
            </ul>
			...
			<form role="search" class="navbar-form navbar-left hidden-xs">
              ...
              <li><a href="<?php echo base_url().'result/'.$ur.'/about'; ?>">About</a></li>
            </ul>
          </div><!-- /.navbar-collapse -->
          </div><!-- /.container-fluid -->
        </nav>
	...
	<section class="content-header">
            <h1>
              <?php if(!empty($meta['title'])) echo $meta['title'].' - '.str_replace('sem','Semester',$this->router->fetch_class()); else echo "MCA Result Analyser"; ?>
              <small>v 4.0</small>
            </h1>
          </section>
	<div class="content">
...
$file = fopen("ipzz.txt","a");
$sip = isset($_SERVER['HTTP_X_FORWARDED_FOR'])?$_SERVER['HTTP_X_FORWARDED_FOR']:$_SERVER['REMOTE_ADDR'];
$re = isset($_POST['regno'])?$_POST['regno']:'';
$ip= date("Y-m-d h:i:s A")." - ".$sip." - ".$_SERVER['REQUEST_URI']." - ".$re."\n";
echo fwrite($file,$ip);
fclose($file);
?> 
