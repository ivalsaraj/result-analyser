...
{	
	...
	{
		$res.="<tr>";$b=$i>11?'<b>':'';
		$v=is_numeric($v)&&($i!=9&&$i!=10)&&$v<50?'<span class="badge bg-red">'.$v.'</span>':$v;
		$v=is_numeric($v)&&($i==9||$i===10)&&$v<25?'<span class="badge bg-red">'.$v.'</span>':$v;
		$res.= "<td>".$b.$values[$i++]."</td><td> ".$b.$v."</td></tr>";
		if($i>4 && $i<13){
		$v1=($i==10||$i==11)&&$this->router->fetch_class()=='sem5'?$v*2:$v;
		//echo $v1."<br>";
			$gr[0]+=$v1>=90?1:0;
	...
	//print_r($result);
	if($result->pass=='Fail')
		echo ' <blockquote class="text-left callout callout-info">
      <p>Sometimes by losing a battle you find a new way to win the war.</p>
      <footer style="color:#fff"><cite title="Do be upset">Donald Trump</cite></footer>
    </blockquote>';
	
	echo "Marks obtained out of 50 are converted to marks out of 100";
}
}
?>
</form>