...
if(isset($sub))
{
	...
	foreach($sub as $c)
	{
		echo "<option value='mca".$id++."'>".$c->subname."</option>";
	}
...
?>
		<div class="callout callout-info">
                    ...
                    <p>eg: click a Subject name to sort subject wise within this list (not from all students), click again to sort in descending order.
		</p>
		</div>
<?php
...
{ //$sub=short_subjects($subject);
	...
	echo '<div class="alert alert-danger alert-dismissable">
                    ...
                    <h4><i class="icon fa fa-info">#</i> Subject :
                  '.$subject.' </h4> Top Score : '.$result->row()->name.' </div>';
	$op='';
	...
	foreach($res as $v)
	{
		if(is_numeric($v) && $v<50) {$v="<span class='badge bg-info'>".$v."</span>";}
		$op.= "<td>".$v."</td>";
	}
	$op.= "</tr>";
	}
