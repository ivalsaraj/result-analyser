<?php
	class Sem4 extends CI_Controller
	{
	...
	function compare($reg1=null)
	{
		...
		if($this->input->post())
		{
			$this->load->model('storeresult4');
			$regs1=$this->input->post('reg1');
			$regs2=$this->input->post('reg2');
			$regres1=$this->storeresult4->get_marks($regs1);
			$regres2=$this->storeresult4->get_marks($regs2);
			$subject=$this->storeresult4->get_subjects();
	...
	function top20()
	{
		...
		$this->output->set_meta('description','Top20 Ranks in Semester IV');
		$this->load->model('storeresult4');
		$result=$this->storeresult4->allmarks(20);
	...
	function top50()
	{
		...
		$this->output->set_meta('description','Top50 Ranks in Semester IV');
		$this->load->model('storeresult4');
		$result=$this->storeresult4->allmarks(50);
		$this->load->view('topmarklistview',array('result'=>$result));
	}
	function top10()
	{
		...
		$this->output->set_meta('description','Top10 Ranks in Semester IV');
		$this->load->model('storeresult4');
		$result=$this->storeresult4->allmarks(10);
		$this->load->view('topmarklistview',array('result'=>$result));
	}
	function myrank()
	{	
		...
		$this->output->set_meta('description','Check your global rank of Semester IV');
		if($this->input->post())
		{
			$id=$this->input->post('regno');
			$result=$this->storeresult4->getmyrank($id)->row();
	...
	function subjectwiserank()
	{
		...
		$this->output->set_meta('description','Subject Wise Ranks of Semester IV');
		$this->load->model('storeresult4');
		$sub=$this->storeresult4->get_subjects()->result();
		//die(var_dump($sub));
		if($this->input->post())
		{
			//$this->load->model('storeresult4');
			...
			if($id!=-1)
			$result=$this->storeresult4->subjectwiserank($id);
	...
	function mycollege()
	{
		...
		$this->load->model('storeresult4');
		$clg=$this->storeresult4->getcollege();
		//die(print_r($result));
		if($this->input->post())
		{
			$this->load->model('storeresult4');
			...
			if($id!=-1)
			$result=$this->storeresult4->getcollegeresult($id);
			$subject=$this->storeresult4->get_subjects()->result();
		}
	...
	function topmark()
	{
		...
		$this->output->set_meta('description','Rank holder - Top mark in Semester IV');
		$this->load->model('storeresult4');
		$result=$this->storeresult4->allmarks(1)->row();
		//die(print_r($res));
		//$result=$this->storeresult4->get_marks($res->regno);
		$subject=$this->storeresult4->get_subjects();
		//$this->output->cache(2);
	...
	function index()
	{
		...
		$this->output->set_meta('description','MCA Semester IV Results');
		$this->db->cache_off();
		$result='';
		$subject='';
		$msg='';
		...
		$result=$this->storeresult4->get_marks($data);
		if(empty($result)) $msg='Invalid Register Number';
		$subject=$this->storeresult4->get_subjects();
		}else
		{
			$this->db->cache_on();
			$result=$this->storeresult4->allmarks(4);
		}
