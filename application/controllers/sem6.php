<?php
	class Sem6 extends CI_Controller
	{
	...
	function compare($reg1=null)
	{
		...
		if($this->input->post())
		{
			$this->load->model('storeresult6');
			$regs1=$this->input->post('reg1');
			$regs2=$this->input->post('reg2');
			$regres1=$this->storeresult6->get_marks($regs1);
			$regres2=$this->storeresult6->get_marks($regs2);
			$subject=$this->storeresult6->get_subjects();
	...
	function top20()
	{
		...
		$this->load->model('storeresult6');
		$result=$this->storeresult6->allmarks(20);
	...
	function top50()
	{
		...
		$this->output->set_meta('description','Top50 Ranks in Semester VI');
		$this->load->model('storeresult6');
		$result=$this->storeresult6->allmarks(50);
		$this->load->view('topmarklistview',array('result'=>$result));
	}
	function top10()
	{
		...
		$this->output->set_meta('description','Top10 Ranks in Semester VI');
		$this->load->model('storeresult6');
		$result=$this->storeresult6->allmarks(10);
		$this->load->view('topmarklistview',array('result'=>$result));
	}
	function myrank()
	{	
		...
		if($this->input->post())
		{
			...
			$result=$this->storeresult6->getmyrank($id)->row();
	...
	function subjectwiserank()
	{
		...
		$this->load->model('storeresult6');
		$sub=$this->storeresult6->get_subjects()->result();
		//die(var_dump($sub));
		if($this->input->post())
		{
			//$this->load->model('storeresult6');
			...
			if($id!=-1)
			$result=$this->storeresult6->subjectwiserank($id);
	...
	function mycollege()
	{
		...
		$this->load->model('storeresult6');
		$clg=$this->storeresult6->getcollege();
		//die(print_r($result));
		if($this->input->post())
		{
			$this->load->model('storeresult6');
			...
			if($id!=-1)
			$result=$this->storeresult6->getcollegeresult($id);
			$subject=$this->storeresult6->get_subjects()->result();
		}
	...
	function topmark()
	{
		...
		$this->load->model('storeresult6');
		$result=$this->storeresult6->allmarks(1)->row();
		//die(print_r($res));
		//$result=$this->storeresult6->get_marks($res->regno);
		$subject=$this->storeresult6->get_subjects();
		//$this->output->cache(2);
	...
	function index()
	{
		...
		$result=$this->storeresult6->get_marks($data);
		//die(var_dump($result));
		if(empty($result)) $msg='Invalid Register Number';
		$subject=$this->storeresult6->get_subjects();
		}else
		{
			$this->db->cache_on();
			$result=$this->storeresult6->allmarks(4);
			$result_overall=$this->storeresult6->allmarks_overall(4);
		}
		$this->load->view('result_regno_sem6',array('result'=>$result,'result_overall'=>$result_overall,'subject'=>$subject,'msg'=>$msg));
	}
	}
?>